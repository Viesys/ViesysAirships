...
public class MessageGuiCustomizeMenuCoreModelFramePg1 extends MessageBase<MessageGuiCustomizeMenuCoreModelFramePg1> {
	...
	{
		player.openGui(ViesysAirships.instance, GuiHandler.GUI_APPEARANCE_MENU_CORE_MODEL_FRAME_PG1, player.world, player.getPosition().getX(), player.getPosition().getY(), player.getPosition().getZ());
	}
}
