package com.viesys.viesysairships.network.server.airship.customize.core;
...
public class MessageGuiCustomizeMenuCoreMain extends MessageBase<MessageGuiCustomizeMenuCoreMain> {
	...
	{
		player.openGui(ViesysAirships.instance, GuiHandler.GUI_APPEARANCE_MENU_CORE_MAIN, player.world, player.getPosition().getX(), player.getPosition().getY(), player.getPosition().getZ());
	}
}
