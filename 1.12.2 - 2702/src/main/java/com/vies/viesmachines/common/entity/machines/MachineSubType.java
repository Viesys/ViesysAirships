...
public class MachineSubType {
    // ========== MachineSubType Global ==========
    ...
    /** Higher priority MachineSubType will always spawn in place of lower priority ones if they can spawn regardless of weight. Only increase this above 0 if a subspecies has conditions otherwise they will stop standard/base MachineSubType from showing up. **/
	...
	/**
	 * Loads global MachineSubType config values, etc.
	 * @param config The config instance to load values from.
	 */
	public static void loadGlobalSettings(ConfigBase config) {
        baseSpeciesWeight = config.getInt("Mob Variations", "MachineSubType Base Weight", baseSpeciesWeight, "The weight of base subspecies (regular mobs).");
        commonWeights.put("uncommon", config.getInt("Mob Variations", "MachineSubType Uncommon Weight", commonWeights.get("uncommon"), "The weight of uncommon subspecies (such as Azure, Verdant, Scarlet, etc)."));
        commonWeights.put("rare", config.getInt("Mob Variations", "MachineSubType Rare Weight", commonWeights.get("rare"), "The weight of rare subspecies (such as Lunar or Celestial)."));

        // Difficulty:
        String[] subspeciesNames = new String[] {"uncommon", "rare"};
		statMultipliers = new HashMap<>();
        config.setCategoryComment("MachineSubType Multipliers", "Here you can scale the stats of every mob on a per subspecies basis.");
        for(String subspeciesName : subspeciesNames) {
            for(String statName : CreatureStats.STAT_NAMES) {
                ...
                statMultipliers.put((subspeciesName + "-" + statName).toUpperCase(), config.getDouble("MachineSubType Multipliers", subspeciesName + " " + statName, defaultValue));
            }
        }


        uncommonDropScale = config.getInt("Mob Variations", "MachineSubType Uncommon Item Drops Scale", uncommonDropScale, "When a creature with the uncommon subspecies (Azure, Verdant, etc) dies, its item drops amount is multiplied by this value.");
        rareDropScale = config.getInt("Mob Variations", "MachineSubType Rare Item Drops Scale", rareDropScale, "When a creature with the rare subspecies (Celestial, Lunar, etc) dies, its item drops amount is multiplied by this value.");

        uncommonExperienceScale = config.getDouble("Mob Variations", "MachineSubType Uncommon Experience Scale", uncommonExperienceScale, "When a creature with the uncommon subspecies (Azure, Verdant, etc) dies, its experience amount is multiplied by this value.");
        rareExperienceScale = config.getDouble("Mob Variations", "MachineSubType Rare Experience Scale", rareExperienceScale, "When a creature with the rare subspecies (Celestial, Lunar, etc) dies, its experience amount is multiplied by this value.");

		uncommonSpawnDayMin = config.getInt("Mob Variations", "MachineSubType Uncommon Spawn Day Min", uncommonSpawnDayMin, "The minimum amount of days before uncommon species start to spawn.");
		rareSpawnDayMin = config.getInt("Mob Variations", "MachineSubType Rare Spawn Day Min", rareSpawnDayMin, "The minimum amount of days before rare species start to spawn.");

		rareHealthBars = config.getBool("Mob Variations", "MachineSubType Rare Health Bars", rareHealthBars, "If set to true, rare subspecies such as the Lunar Grue or Celestial Geonach will display boss health bars.");
    }


    public static MachineSubType createFromJSON(JsonObject json) {
		// Rarity:
		...
		MachineSubType subspecies = new MachineSubType(skin, color, rarity);
	...
	/**
	 * Constructor for creating a color/skin MachineSubType based on a rarity.
	 * @param skin The skin of the MachineSubType. Can be null or default skin.
	 * @param color The color of the MachineSubType. Can be null for default color.
	 * @param rarity The rarity of the MachineSubType ('common', 'uncommon' or 'rare'.
	 */
	public MachineSubType(@Nullable String skin, @Nullable String color, String rarity) {
        this.color = color;
        this.skin = skin;
        this.rarity = rarity;
	...
	/**
	 * Gets the display name of this MachineSubType.
	 * @return The MachineSubType title.
	 */
	...
	/**
	 * Returns if this MachineSubType is allowed to be used on the spawned entity.
	 * @return True if this MachineSubType is allowed.
	 */
