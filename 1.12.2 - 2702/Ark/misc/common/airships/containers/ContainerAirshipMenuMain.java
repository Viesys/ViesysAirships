package com.viesys.viesysmachines.common.entity.airships.containers;
...
import com.viesys.viesysmachines.common.entity.airships.slots.FuelSlotVC;
import com.viesys.viesysmachines.common.entity.airships.slots.InventorySlotVC;
...
public class ContainerAirshipMenuMain extends ContainerVC {
	
	private EntityMachineAirship airship;
    
	public ContainerAirshipMenuMain(IInventory playerInv, EntityMachineAirship airshipIn) 
	{
		...
		ArrayList<ResourceLocation> nums = new ArrayList<ResourceLocation>();
		
		ArrayList<ResourceLocation> numsnew = new ArrayList<ResourceLocation>();
		
		nums.addAll(SoundEvent.REGISTRY.getKeys());//).add(SoundEvent.REGISTRY);
		
		for(int i = 0; i < nums.size(); i++)
		{
			if(nums.get(i).toString().toLowerCase().contains("record.".toLowerCase()))
		    {
		    	numsnew.add(nums.get(i));
		    }
		}
		
		LogHelper.info(numsnew);
		...
		//Slot 0 - Fuel
		this.addSlotToContainer(new FuelSlotVC(this.airship.inventory, 0, 141, 30));
